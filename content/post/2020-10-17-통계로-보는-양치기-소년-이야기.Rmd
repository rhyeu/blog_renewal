---
title: 통계로 보는 양치기 소년 이야기
author: 류성균
date: '2020-10-17'
slug: 통계로-보는-양치기-소년-이야기
categories:
  - essay
tags:
  - bayesian
...
![wolf](C:\Users\rhyeu\Documents\GitHub\blog_renewal\content\img\202011_sheep_keeper)

양을 치는 소년이 심심풀이로 "늑대가 나타났다!"라고 거짓말을 하고 소란을 일으킨다. 그 동네의 어른들은 소년의 거짓말에 속아 무기를 가져오지만, 헛수고로 끝난다. 양치기 소년은 이런 거짓말을 여러 번 반복했다. 결국 어느 날에 정말 늑대가 나타났지만 어른들은 더 이상 양치기 소년의 말을 믿지 않았고, 아무도 도우러 가지 않았다. 결국 양치기 소년의 모든 양이 늑대에 의해 잡아먹혔다.(출처 - [위키백과](https://ko.wikipedia.org/wiki/%EC%96%91%EC%B9%98%EA%B8%B0_%EC%86%8C%EB%85%84))

- 양치기 소년이 10일 동안 양을 치고 있었고, 매일 "늑대가 나타났다."라고 거짓말을 하며 사람들을 불러모았다고 하자. 그 중 8번은 늑대가 나타나지 않았지만 9일차, 10일차에는 늑대가 나타났다고 가정해본다. 이를 표현하면 아래와 같다. 이를 가지고 소년이 거짓말쟁이인지 통계적으로 판단해보자. 방법은 **전통적인 이항검정**, **베이즈 정리를 이용한 추정**, **베이지안 사후확률**을 구하는 방법 순으로 해보고자 한다.
...
$H_0 : \pi = 0.9$ => (소년이 진실을 말할 확률 $\pi$는 0.9이다.)

이를 판단하기 위한 검정에는 여러 가지가 있겠지만, 매번 늑대가 나타날 확률이 독립적이라고 할 경우 이항 검정을 통해 가설 검정을 할 수 있다. 먼저 이항 검정의 가정은 아래와 같다.(이 때, $\pi_0$은 영가설 하에서의 $\pi$를 의미한다.)
...
- 덧붙여 이항시행에서 아쉬운 점은 소년의 보고(거짓말 / 참말)을 독립시행이라고 봤기 때문에 거짓말이나 참말의 순서를 전혀 고려하지 못하게 된다.
...
- 그럼 이번엔 베이즈 정리를 이용해 10번의 시행마다 거짓말을 할 확률이 어떻게 변화하는 지 확인해보자.(이 부분은 월간 뉴턴의 내용을 인용함.)
- 소년이 거짓말 쟁이일 확률을 0.1, 정직할 확률을 0.9로 두자.(이를 관측을 하기 전에 가정한다는 의미로 '사전확률'이라고 한다.) 또한, 소년이 거짓말쟁이일 때 늑대가 발견될 확률을 0.3, 소년이 거짓말쟁이일 때 늑대가 발견될 확률을 0.7로 보자. 이러한 내용은 아래의 수식으로 표현할 수 도 있다.
...
- 이제 소년이 거짓말쟁이일 확률을 구해보자, 첫번 째 날, 소년이 "늑대가 나타났다."고 말했지만 늑대가 나타나지 않았다. 가능한 경우의 수는 두 가지 인데, 소년이 정직하게 말했지만 늑대가 보이지 않았거나, 소년이 거짓말쟁이어서 늑대가 없었던 것이다. 먼저, 소년이 거짓말을 했을 확률은 아래와 같은 계산을 통해 `r lie[1]*mat["거짓","미발견"] / (lie[1]*mat["거짓","미발견"] + honest[1]*mat["정직","미발견"])`이 나온다. 
...
- 늑대가 나타나지 않자, 소년이 거짓말쟁이일 확률은 0.1에서 0.28로 2배 넘게 올라갔다. 그렇다면 8번의 경우에 따라 소년이 거짓말을 할 확률도 모두 계산해볼 수 있다. 흥미로운 점은 이미 여러 차례 늑대가 발견되지 않았기 때문에, 7번째와 8번째 시행에서 늑대가 나타났다고 하더라도 거짓말할 확률은 크게 바뀌지 않았다는 점이다.
...
- 베이지안 통계학에서 흥미로운 점은 사전확률을 바꿀 수 있다는 점이다. 만약에 사전 확률이 균등분포가 아니라 소년이 정직하다는 것에 가중치를 둘 경우($\alpha = 1, \beta=10$) 분포의 변화는 아래와 같다. 사전분포는 확률이 1에 치우친 반면
